...
def predict(data: Union[str, List[str], Dict[str, Any]]) -> Dict[str, Any]:
    """
    ...
    print(f"[Inference] Received prediction request, data type: {type(data)}")
    ...
    if isinstance(data, str):
        ...
        reviews = data.get('reviews', data.get('input', data.get('text', [])))
        if isinstance(reviews, str):
            reviews = [reviews]
    else:
        raise ValueError(
            f"Invalid input format. Expected string, list, or dict, got {type(data)}\n\n"
            f"Valid formats:\n"
